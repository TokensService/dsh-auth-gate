...
import { wrapServer } from "./gate/index.js";
import { makeSessionTtlResolver, PasswordGate, registerManagementEndpoints, registerPasswordEndpoints, verifyPassword, } from "./features/password/index.js";
import { generateTotpSecret, totpUri, TotpReplayGuard, verifyTotpCode, } from "./features/totp/index.js";
import { LoginRateLimiter, defaultSettingsFilePath, defaultUsersFilePath, loadUsersFile, } from "./shared/index.js";
 ...
 */
function makeTokenResolver(ctx, config, log) {
 ...
 */
function mountSessionDomain(ctx, auth, log) {
 ...
 */
function mountAuthEndpoints(server, config, auth, resolveToken, launchTokenBridge, usersPath, limiter, replayGuard, challengeMacKey, log) {
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => mountAuthEndpoints(server, config, auth, resolveToken, launchTokenBridge, usersPath, limiter, replayGuard, challengeMacKey, log), "dsh-auth-gate: auth endpoints");
