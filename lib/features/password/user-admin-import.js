...
function dispatch(deps, req, res) {
    if (req.method === "GET")
        return handleListFiles(deps, req, res);
    if (req.method === "POST")
...
async function handleListFiles(deps, req, res) {
    ...
    for (const dirent of dirents) {
        if (!dirent.isFile() || !SERVER_FILE_RE.test(dirent.name))
            continue;
        try {
...
async function handleImport(deps, req, res) {
    ...
    if ((text === undefined) === (file === undefined))
        return sendCode(res, 400, "invalid_field");
    let content;
    if (text !== undefined) {
        content = text;
    }
    ...
    if (!(await writeUsersOr503(deps, res, admin.snapshot)))
        return;
    deps.logger.info(`imported ${entries.length} users via /auth/users/import (${text === undefined ? `server file ${file}` : "inline text"})`);
    sendJson(res, 201, {
...
async function readServerImportFile(deps, res, name) {
...
function importsDirOf(usersPath) {
