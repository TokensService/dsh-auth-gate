...
都可 fetch 此端点来显示当前登录用户。

client 包还常驻一个会话看门狗：启动后立即探测 `/auth/status`，有限会话按
`expiresAt` 在浏览器本地定时，并保留每 30 秒、标签页重新获得焦点或恢复可见时的
探测作为吊销兜底。一旦会话过期（或在另一个标签页被吊销），页面会主动跳回登录页，
而不是一直停留在已登录的旧界面。暂时性探测失败（服务器重启、网络抖动）不会触发
跳转，也不会取消已安排的过期定时器。
