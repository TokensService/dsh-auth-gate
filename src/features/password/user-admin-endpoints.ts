 ...
 */
export function registerUserAdminEndpoints(deps: UserAdminDeps): () => void {
...
function dispatch(
  ...
  if (req.method === "GET") return handleList(deps, req, res);
  if (req.method === "POST") return handleCreate(deps, req, res);
  if (req.method === "PATCH") return handleUserUpdate(deps, req, res);
  if (req.method === "DELETE") return handleUserDelete(deps, req, res);
...
async function handleCreate(
  ...
  if (password === "") return sendCode(res, 400, "empty_password");
  const disabled = body["disabled"] === true;
  const loaded = await loadUsersOr503(deps, res);
  if (loaded === undefined) return;
  if (!isAdmin(loaded.snapshot, subject)) return sendCode(res, 403, "forbidden");
  if (loaded.snapshot.users.has(username)) return sendCode(res, 409, "duplicate");
